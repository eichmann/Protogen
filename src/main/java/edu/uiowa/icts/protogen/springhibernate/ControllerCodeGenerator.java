...
public class ControllerCodeGenerator extends AbstractSpringHibernateCodeGenerator {
	...
	private void generateController( DomainClass domainClass ) throws IOException {
		...
		String packageName = model.getPackageRoot() + "." + domainClass.getSchema().getLowerLabel() + ".controller";
		
		// String daoPackageName = model.getPackageRoot() + "." + domainClass.getSchema().getLowerLabel() + ".dao";
		// String accessor = domainClass.getSchema().getLowerLabel() + "DaoService.get" + domainClass.getIdentifier() + interfaceSuffix + "()";
		...
		//
		//		importList.add( "import " + domainClass.getPackageName() + ".*;" );
		...
		//		out.write( "@RequestMapping( \"/" + domainClass.getSchema().getLowerLabel().toLowerCase() + "/" + domainClass.getLowerIdentifier().toLowerCase() + "/*\" )\n" );
		//		out.write( "public class " + className + " extends Abstract" + domainClass.getSchema().getUpperLabel() + "Controller {\n\n" );
		//
		//		spaces( out, 4 );
		//		out.write( "private static final Log log = LogFactory.getLog( " + className + ".class );\n\n" );
		//
		//		out.write( generateNoScriptListMethod( domainClass, accessor, jspPath, 4 ) );
		//		lines( out, 2 );
		//		out.write( generateListMethod( domainClass, accessor, jspPath, 4 ) );
		//		lines( out, 2 );
		//		out.write( generateDataTableMethod( domainClass, accessor, jspPath, 4 ) );
		//		lines( out, 2 );
		//		out.write( generateAddMethod( domainClass, accessor, jspPath, 4 ) );
		//		lines( out, 2 );
		//		out.write( generateEditMethod( domainClass, accessor, jspPath, 4 ) );
		//		lines( out, 2 );
		//		out.write( generateShowMethod( domainClass, accessor, jspPath, 4 ) );
		//		lines( out, 2 );
		//		out.write( generateSaveMethod( domainClass, accessor, jspPath, 4 ) );
		//		lines( out, 2 );
		//		out.write( generateDeleteMethodGET( domainClass, accessor, jspPath, 4 ) );
		//		lines( out, 2 );
		//		out.write( generateDeleteMethodPOST( domainClass, accessor, jspPath, 4 ) );
		//		lines( out, 1 );
		//		out.write( "}" );
		//		out.close();
	}
